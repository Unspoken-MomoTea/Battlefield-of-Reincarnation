...
- “发现”页采用工坊式卡片浏览，左侧提供“全部项目 / 扩展 / 角色 / 开局商店”；“角色”进入独立首页并分为世界书角色、开局角色、开局伙伴三栏。目录可按最新、热门、下载、收藏或点赞排序，并使用服务端分页。
...
- 创建作品只有一个顶层“分类”选择：扩展 / 角色 / 开局商店。选择“角色”后才出现“世界书角色 / 开局角色 / 开局伙伴”子类型；不会同时出现互相冲突的“扩展用途”和“角色用途”。
- 世界书角色改为“姓名 / 关键词 + 单一世界书正文面板”：正文预填角色设定模板，作者可自由增删栏目；不再单独维护身份、职业、层级等 MVU 式字段。生成条目固定使用“角色定义后（↓ Char）”，概率 100；600 仅作为角色区间基准，实际安装时按当前已安装角色 MOD 的顺序动态分配 601、602、603……。更新同一 MOD 保留原相对位置，卸载后自动前移补空位；非 @D 插入不再携带无意义的 depth/D4。旧版世界书角色在再次编辑时会把原种族、身份、职业、层级、性格、外貌、背景故事与补充设定迁移进自由正文，避免资料丢失。开局角色/开局伙伴仍使用表单维护开局构筑，层级仅允许Ⅰ-Ⅲ，血统与技能品质由人物层级自动锁定为Ⅰ→F、Ⅱ→E、Ⅲ→D。
- 开局角色五维使用与正式开局相同的0-8点→F-SSS映射，总预算8点；开局伙伴总预算16点。编辑器复用正式开局的“− / 当前品质 / +”横向加点方式，“原始构筑 · 血统与五维”和剩余点数同排显示。两者都只允许1项血统与最多2项技能；技能通过“添加技能/删除技能”动态维护，伙伴额外可动态添加装备。所有构筑均通过普通表单填写，不暴露 JSON/JS 编辑框。
- 开局伙伴的人设区在“背景故事”下方提供可选“填写世界书”按钮。开启后使用与“世界书角色”完全相同的自由世界书编辑方式：填写关键词/别名，并在同一套角色设定模板中直接编辑正文；不会把上方 MVU 的种族、身份、层级、性格、喜爱或背景故事自动写入世界书。伙伴姓名会自动加入绿灯主关键词，作者填写的关键词作为额外绿灯关键词。发布后生成“角色定义后”的角色世界书，并与世界书角色共用 601、602、603……的动态安装顺序；未开启或正文为空时不生成 worldbook artifact。
- 正式开局选择已安装的开局角色后，会锁定底部手工血统加点面板，并在确认页直接展示该角色资产内的层级、血统品质和五维，避免再次加点或显示错误摘要。
- 开局商店使用可增删的商品卡片编辑器，每项先选择装备 / 道具 / 技能。商品品质仅允许 F / E / D，价格区间分别为 F=50-1000、E=300-1000、D=700-1000；装备原始属性可填写 F-A，道具另外填写数量；每项商品最多2条效果，通过动态“添加效果/删除”按钮维护（第一条同样可删除）。
- “原版资源状态”只属于通用扩展。世界书角色、开局角色、开局伙伴和开局商店均不会显示、保存或应用原版世界书 / 正则 / 脚本状态规则。
- 作者可从当前 MVU 直接发布“异端库”角色；上传会递归剔除最终属性、真属性、HP/EP，并排除道具、货币、凭证、任务、成就与世界状态，仅保留原始战斗构筑及作者补充的人设。
- 开局商店虽然服务端仍复用 extension 顶层协议，但作者界面把它作为独立分类；商品卡片自动生成 store_catalog，安装后合并进开局商店。商品 ID 会按来源项目命名空间隔离，升级/卸载与安装事务同步回滚。
...
- 所有发布作品都必须有封面图片；开局角色/伙伴的封面同时作为默认头像。本地测试同样要求图片以保持与正式发布一致；已有作品更新可继续沿用现有封面，只有明确选择新图片才替换。
...
- 开局角色 / 开局伙伴作品的封面同时作为默认头像来源。安装后 Opening Asset 会记录对应封面地址。
- 正式开局选择开局角色后，会在初始化时写入状态栏玩家头像键；选择开局伙伴后会按伙伴名称写入关系面板 NPC 头像键。
- 这只是开局默认值：状态栏原有的点击头像上传、URL 替换、清除等功能保持不变，玩家后续修改会覆盖默认头像。
- 本地测试作品同样会把当前封面转为本地头像数据，便于作者不上传服务器就完整测试角色/伙伴效果。


## 开局商店在开局页的落点

- 已安装的 `store_catalog` 会在开局页读取 `opening_store_catalogs` 并合并进现有装备 / 道具 / 技能数据库。
- 每个装备 / 道具 / 技能页的左侧子分类会在存在工坊商品时额外出现“创意工坊”，用于集中查看已安装商品；商品仍保留原本类型数据，选中后走现有购买与 MVU 写入流程。
- 品质筛选只显示当前子分类真实存在商品的品质；没有商品的品质按钮不会渲染，仅有一种品质时不显示冗余“全部”按钮。
- 自定义伙伴切换后直接显示建档表单，不再额外嵌套“招募自定义伙伴”折叠栏。


## 开局商店字段格式

作者不直接填写 JSON。工坊表单按开局现有数据库字段生成 Catalog：

- 装备：名称、品质、价格、装备类型（18类）、标签、原始属性、最多2条效果、描述、消耗。ID 与来源由工坊自动生成。
