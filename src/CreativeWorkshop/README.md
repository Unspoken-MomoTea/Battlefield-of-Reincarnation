...
- 发布界面只保留明确的“世界书 / 正则 / 酒馆助手脚本”上传入口，不再暴露“其他文件 / 预设 / 完整 bundle”这种含义不清的通用按钮。
- 点击“下一步”后才进入安装规则：世界书选择器按世界推进同样的来源规则读取“当前角色主书 / 角色附加 / 当前聊天绑定 / 全局启用”，并且只显示当前启用条目；条目会标出蓝灯常驻、绿灯关键词或向量化状态，并可展开查看正文。
- 酒馆助手脚本替换选择同样只显示当前启用脚本；世界书/脚本的原版目标都不要求作者手写 UID、脚本 ID 或作用域。
- 已发布作品的“更新作品”与首次发布使用同一套编辑结构：名称、简介、标签、依赖、当前封面和上一版全部 artifact 自动载入。上传同名世界书/正则/脚本会替换上一版对应项，未修改的旧内容继续保留，不需要重新拼完整版本。
