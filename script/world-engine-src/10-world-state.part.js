    ...
    function retryInput(baseInput,error,lastReply,attempt,maxAttempts,acceptedResult,retryPlan=[],promptService=null) {
        ...
        const prompt=(key,fallback,vars={})=>promptService&&typeof promptService.get==='function'?promptService.get(key,vars):worldEditablePromptText(key,fallback,vars);
        payload.纠错重试={
            当前尝试:attempt+1,
            最大尝试次数:maxAttempts,
            ...
            要求:acceptedResult
                ?(plan.length
                    ?prompt('retry.acceptedWithPlan','严格按“补充清单”只补充或修正未通过的业务片段。已接受业务结果已经通过本地验收，默认全部保留，不要整份重写；同名实体只提交需要覆盖的字段。若某个本轮提案应撤回，用 操作=撤销本轮。仍只输出一个 WorldResult JSON。')
                    :prompt('retry.acceptedWithoutPlan','只补充或修正导致拒绝的业务片段。已接受业务结果默认保留，不要整份重写；同名实体只提交需要覆盖的字段。若某个本轮提案应撤回，用 操作=撤销本轮。仍只输出一个 WorldResult JSON。'))
                :prompt('retry.fresh','修正格式或业务错误后重新输出一个 WorldResult JSON；不要解释错误，不要输出存储路径。')
        };
