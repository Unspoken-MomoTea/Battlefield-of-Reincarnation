...
import {
  categoryField, getOwnedProject, nowSeconds, optionalText, parseTags,
  slugField, tagsField, textField,
} from '../core.js';
...
export async function createProject(request, env, user) {
  ...
  await env.DB.prepare(
    `INSERT INTO projects
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, 'draft', 0, 0, ?, ?)`,
  ).bind(id, user.id, slug, name, summary, JSON.stringify(tags), category, now, now).run();
  return json({ project: { id, slug, name, summary, tags, category, status: 'draft', latest_version: 0, published_version: 0, created_at: now, updated_at: now } }, 201);
}
...
export async function updateProject(request, env, user, projectId) {
  ...
  const next = {
    ...
    category: category ?? project.category, tags: tags ?? parseTags(project.tags),
  };
  const now = nowSeconds();
  await env.DB.prepare('UPDATE projects SET name = ?, summary = ?, tags = ?, category = ?, updated_at = ? WHERE id = ?')
    .bind(next.name, next.summary, JSON.stringify(next.tags), next.category, now, projectId).run();
  ...
  if (Number(project.latest_version) > 0) {
    ...
    if (latest && ['draft', 'rejected'].includes(latest.review_status)) {
      await env.DB.prepare('UPDATE project_versions SET name = ?, summary = ?, tags = ?, category = ? WHERE project_id = ? AND version = ?')
        .bind(next.name, next.summary, JSON.stringify(next.tags), next.category, projectId, project.latest_version).run();
    }
  }
  return json({ ok: true });
}
