...
- 发布界面保留明确的“世界书 / 正则 / 酒馆助手脚本 / 开局数据”上传入口；开局数据只用于结构化的开局角色、开局伙伴与商店 Catalog。
- 角色作品可选择“世界书人物 / 开局角色 / 开局伙伴”。开局角色与伙伴必须携带有效 build，安装后进入共享 Opening Asset Registry，卸载/升级会同步移除或替换。
- 作者可从当前 MVU 直接发布“异端库”角色；上传会递归剔除最终属性、真属性、HP/EP，并排除道具、货币、凭证、任务、成就与世界状态，仅保留原始战斗构筑及作者补充的人设。
- 扩展作品可携带 store_catalog 数据，安装后合并进开局商店；商品 ID 会按来源项目命名空间隔离，升级/卸载与安装事务同步回滚。
