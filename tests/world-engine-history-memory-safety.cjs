...
function stateWithHistory(count=18){
  const backend=emptyState();
  for(let i=1;i<=count;i++)backend.历史['历史'+String(i).padStart(3,'0')]={时间:'第'+i+'日',事实:'已确认事实'+i,关联事件:[]};
  return {
    世界:{名称:'历史安全测试',时间:'第30日',地点:'城内',稳定:100,后台:backend,因果轨道:{当前阶段:'局势延续',故事线:'',下一节点:'',偏移记录:{}},异端雷达:{名单:{}},势力:{},探索:{},法则:[],货币:{},历法:{}},
...
(async()=>{
  // 历史总结是附加维护：它失败时不能撤销已经成功落库的主世界推进。
  {
    ...
    const host={
      ...
      Samsara:{terminal:{apiReady:()=>true,request:async()=>{
        calls++;
        return calls===1?JSON.stringify({摘要:'本轮世界正常推进。'}):'这不是合法的历史总结 JSON';
    ...
    assert.equal(calls,2,'达到阈值后应尝试一次历史总结');
