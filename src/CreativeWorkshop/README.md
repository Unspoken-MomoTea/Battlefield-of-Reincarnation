...
- 世界书角色改为“姓名 / 关键词 + 单一世界书正文面板”：正文预填角色设定模板，作者可自由增删栏目；不再单独维护身份、职业、层级等 MVU 式字段。生成条目固定使用“角色定义后（↓ Char）”，顺序 600、概率 100；非 @D 插入不再携带无意义的 depth/D4。旧版世界书角色在再次编辑时会把原种族、身份、职业、层级、性格、外貌、背景故事与补充设定迁移进自由正文，避免资料丢失。开局角色/开局伙伴仍使用表单维护开局构筑，层级仅允许Ⅰ-Ⅲ，血统与技能品质由人物层级自动锁定为Ⅰ→F、Ⅱ→E、Ⅲ→D。
...
- 开局伙伴的人设区在“背景故事”下方提供可选“填写世界书”按钮。开启后使用与“世界书角色”完全相同的自由世界书编辑方式：填写关键词/别名，并在同一套角色设定模板中直接编辑正文；不会把上方 MVU 的种族、身份、层级、性格、喜爱或背景故事自动写入世界书。伙伴姓名会自动加入绿灯主关键词，作者填写的关键词作为额外绿灯关键词。发布后生成“角色定义后 / 顺序600”的角色世界书；未开启或正文为空时不生成 worldbook artifact。
