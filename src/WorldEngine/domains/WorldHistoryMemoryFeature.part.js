    class WorldHistoryMemoryFeature {
        ...
        initialize(){
            ...
            engine.historyMaintenanceBusy=false;
            engine.lastHistoryMaintenance='';
            if(dirty)engine.saveConfig();
        }
        setSendToProse(value){
            const engine=this.engine;
            engine.config.sendHistoryToProse=value===true;
            engine.saveConfig();engine.render();
            return engine.config.sendHistoryToProse;
        }
        prose(stat){return projectWorldHistoryMemory(stat?.世界?.[PATH]||{});}
        bindPanel(){
            ...
            panel.addEventListener('click',event=>{
                ...
                this.setSendToProse(this.engine.config.sendHistoryToProse!==true);
            });
        }
        async requestSummary(world,batch,outputLevel){
            ...
            try{
                const raw=await engine.requestAI(
                    ...
                    engine.services?.prompts?.historyInput?.(historyMemoryPrompt(world,batch,outputLevel))||historyMemoryPrompt(world,batch,outputLevel),
        ...
        beforeWorldCommit(next,context={}){
            const engine=this.engine;
            const summary=String(context.worldResult?.摘要||context.reply?.summary||engine.lastWorldResult?.摘要||'').trim();
        ...
        async maintain(){
            const engine=this.engine;
            if(engine.historyMaintenanceBusy)return 0;
            engine.historyMaintenanceBusy=true;
            ...
            try{
                ...
                for(let level=0;level<32;level++){
                    ...
                    engine.status='整理长期历史记忆 · L'+outputLevel;engine.render();
                    let summary='';
                ...
                if(historyMemoryDigest(currentBackend)!==startDigest){
                    engine.lastHistoryMaintenance='历史在总结期间已变化，本次总结结果丢弃，下轮重试';
                    return 0;
                }
                const validate=engine.host.Samsara&&engine.host.Samsara.validateWorldState;
                const next=validate?validate(stat):stat,result=current.raw;result.stat_data=next;
                engine.committing=true;
                await current.mvu.replaceMvuData(result,{type:'message',message_id:current.id});
                engine.lastHistoryMaintenance='新增 '+made+' 个历史总结节点';
                return made;
            }finally{
                ...
                engine.render();engine.historyMaintenanceBusy=false;
            }
        }
