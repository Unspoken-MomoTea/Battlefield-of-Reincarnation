...
const {SamsaraWorldEngine: Engine, applyPatches, emptyState, RECORDS, parseReply, compileWorldResult, WORLD_RESULT_SCHEMA, projectWorldContext, compactWorldLifecycle, calendarDate, repairExplorationGranularity, sortWorldEvents, eventScheduleLabel, staleActiveEvents, temporalAnomalies, extractWorldProse} = require(file);
...
(async () => {
    ...
    await test('world prose strips paired and prefill-only hidden planning blocks', () => {
        assert.equal(
            extractWorldProse('<konatan_planning~>\n绝对不能发送的思考\n</konatan_planning~~>\n真正正文'),
            '真正正文'
        );
        assert.equal(
            extractWorldProse('Master，小此已经切换到日本語进行思考啦！\n- 当前什么情况？\n</konatan_planning~~>\n<bbs_start>斗罗历2643年·12月·20日·申时四刻</bbs_start>\n真正正文'),
            '斗罗历2643年·12月·20日·申时四刻\n真正正文'
        );
        assert.equal(
            extractWorldProse('<think>隐藏</think>\n保留正文\n</think>\n继续正文'),
            '保留正文\n\n继续正文'
        );
    });
    ...
    await test('WorldResult.关系 sparsely updates existing NPC state without creating new NPCs', () => {
        const stat=fresh();
        stat.关系列表.剧情角色={
        ...
        assert.ok(compiled.warnings.some(x=>x.includes('关系对象不存在')));
        const next=applyPatches(stat,compiled.patches);
        ...
        assert.equal(next.关系列表.剧情角色.HP_MAX,120,'关系接口不得覆盖HP_MAX');
        const legacy=compileWorldResult(stat,{摘要:'旧格式',关系:{剧情角色:15}});
        assert.ok(legacy.patches.some(p=>p.path==='/关系列表/剧情角色/好感度'&&p.value===15),'旧关系对象简写仍需兼容');
    });
    ...
    await test('macro acceptance demotes obvious local actions before counting backbone nodes', async () => {
        ...
        x.engine.config.requireMacroBackbone=true;x.engine.config.retryAttempts=3;
        assert.equal(await x.engine.run(),true);
        assert.equal(calls,2);
    ...
    await test('active alien roster members are mandatory world-person activities and dead aliens cannot resurrect', async () => {
        ...
        x.engine.config.requireMacroBackbone=false;x.engine.config.retryAttempts=3;
        assert.equal(await x.engine.run(),true);
    ...
    await test('built-in default prompt document is seeded, visible, and applied on first load', () => {
        ...
        assert.equal(engine.config.builtinDefaultPromptVersionApplied,8);
        ...
        assert.match(engine.config.preset,/场外人物/);
    ...
    await test('built-in default keeps NPC build rules selected and excludes only task rules', async () => {
        let stored='';
        const host={
        ...
        assert.equal(selectedIds.includes('196248'),false,'任务与委托系统继续默认取消');
        for(const id of ['503929','931853','446543'])assert.equal(selectedIds.includes(id),true,'NPC构筑相关规则应恢复默认勾选：'+id);
        assert.deepEqual(new Set(engine.config.builtinDefaultWorldbookExclusionsApplied),new Set(['任务与委托系统']));
        ...
        assert.equal(JSON.parse(stored).builtinDefaultWorldbookExclusionsApplied.length,1,'迁移结果只记录任务规则排除');
    });
    ...
    await test('auxiliary callback skips duration ticks for engine commit but processes subsequent prose', () => {
        ...
        assert.equal(calls.guardTaskGenerationLock,1,'世界提交仍要执行任务数据守卫');
        ...
        assert.equal(calls.processStatusDuration,undefined,'世界提交不能额外消耗状态持续时间');
        ...
        for (const engineOn of [false,true]) {
            for (const space of [false,true]) {
                ...
                if(engineOn&&!space){
                    ...
                    const offstageScenes=readonly.世界.场外场景||[];
                    ...
                    assert.equal(readonly.世界.场外场景,undefined);
