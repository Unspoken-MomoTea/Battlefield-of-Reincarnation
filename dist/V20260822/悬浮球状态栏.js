...
(function () {
    ...
    function buildCSS(th, themeKey) {
        ...
        .sam-hold-types { display:flex; flex-wrap:nowrap; gap:4px; padding:2px 10px 8px; margin-bottom:4px; border-bottom:1px dashed var(--sam-border); overflow-x:auto; overflow-y:hidden; flex-shrink:0; -webkit-overflow-scrolling:touch; scrollbar-width:thin; }
        .sam-hold-types::-webkit-scrollbar { height:3px; }
    ...
    function renderMissionTab(sd) {
        ...
        if (achKeys.length === 0) {
            ...
            achKeys.forEach(function(k) {
                ...
                var done = (statusVal === '已达成' || statusVal === '可结算');
                // 编辑模式: 标题区追加删除按钮(挂 data-del-path, 复用通用删除事件)
                var delBtn = editMode ? samDelBtn(path, editMode, '删除该成就') : '';
                // 已达成/可结算: 头部金色✓徽章 + 卡片金色描边(一眼区分达成进度)
                ...
                var statusCell = editMode
                    ? editSelect(path+'.状态', ['进行中','未达成','已达成','可结算'], statusVal)
                    : esc(statusVal);
                var statusRow = '<div class="sam-row"><span class="k">状态</span><span class="v">'+statusCell+'</span></div>';
                var rows = '';
                rows += statusRow;
