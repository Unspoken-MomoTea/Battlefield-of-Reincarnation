...
(async()=>{
 ...
 stat.资产={联防仓库:{所属对象:['<user>','灰港商会','守备官艾琳'],类型:'固定地产',完整度:85,状态:'联防',驻扎人员:{守卫:{人数:3}},建设序列:{},待办事件:[]},无主遗迹:{所属对象:[],类型:'固定地产',完整度:0,状态:'废弃'}};
 ...
 await audit.locator('..').locator('summary').click();
 await audit.fill('自定义 NPC 审计要求：仅补真实缺口。');
 ...
 await page.locator('[data-tab="角色管理"]').click();
 assert.equal(await page.getByRole('heading',{name:'NPC构筑审计',exact:true}).count(),1);
 await page.evaluate(()=>Samsara.worldEngine.setNpcBuildAuditEnabled(false));
 assert.equal(await page.getByRole('heading',{name:'NPC构筑审计',exact:true}).count(),0);
 await page.evaluate(()=>Samsara.worldEngine.setNpcBuildAuditEnabled(true));
 assert.equal(await page.getByRole('heading',{name:'NPC构筑审计',exact:true}).count(),1);
 await page.locator('[data-tab="设置"]').click();
 assert.equal(await page.locator('[data-npc-audit-setting] h2 .we-pill').textContent(),'实验性功能');
