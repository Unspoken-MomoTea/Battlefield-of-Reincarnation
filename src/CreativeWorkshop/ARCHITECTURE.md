...
1. 远程作品始终是“数据”，不获得任意 JavaScript 执行权。
...
- 服务端拒绝 `script` artifact。
- 世界书、正则、预设在写入酒馆前再次解析/规范化。
- 正则使用项目命名空间 ID。
- 预设名带项目 ID 与 artifact 序号。
- 安装失败会回滚已执行步骤。
- 已安装角色相关资源不能在另一角色上误更新/误卸载。
- 安装前预检只提示或阻止真实冲突，不替玩家自动删除第三方资源。
- Repair 只重建当前项目自己拥有的世界书条目、正则和预设，并保留玩家/其他项目资源。
...
1. IndexedDB 多标签页升级协调。
2. 真实 staging 部署后的端到端测试。
