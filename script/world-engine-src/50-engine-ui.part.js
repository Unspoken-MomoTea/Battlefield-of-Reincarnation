        ...
        render(force) {
            ...
            if(this.tab==='世界推进'){
                ...
                html+=this.views.events.render({events,matched,tools,section,timelineCards,empty});
            }else if(this.tab==='传闻'){
                html+=this.views.rumor.render({s,state,tools,section,entries,matched,text,fields,details,empty,pill});
            }else if(this.tab==='运行记录'){
                html+=this.views.history.render({state,radar,showRadar,exists,section,text,entries,fields,empty,pill});
            }else if(this.tab==='设置'){
                html+=this.views.settings.render({section,text});
            }else if(this.tab==='提示词预设'){
                html+=this.views.prompts.render({text,section,empty});
            }else if(this.tab==='请求检查'){
                html+=this.views.inspector.render({text,section,empty,fields,pill});
            }
