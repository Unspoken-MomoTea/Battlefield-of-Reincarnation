...
`WorldPromptRegistry` 是世界推进所有**静态 AI 指令文本**的唯一编辑入口。提示词预设页的“全部实际提示词”直接读取这个注册表，不再依赖人工记住某个 `59-*.part.js` 里还藏着什么字符串。开启编辑后，注册表中的每一项——包括主预设、核心约束、宏观骨架、NPC 审计和输出协议——都能在这一个总清单里直接修改；不再只有只读镜像。
