...
- 世界书角色改为“姓名 / 关键词 + 单一世界书正文面板”：正文预填角色设定模板，作者可自由增删栏目；不再单独维护身份、职业、层级等 MVU 式字段。生成条目固定放在“角色定义后（↓ Char）”的角色区间，顺序 650、概率 100，位于项目世界书的“角色开始 600 / 角色结束 700”之间。旧版世界书角色在再次编辑时会把原种族、身份、职业、层级、性格、外貌、背景故事与补充设定迁移进自由正文，避免资料丢失。开局角色/开局伙伴仍使用表单维护开局构筑，层级仅允许Ⅰ-Ⅲ，血统与技能品质由人物层级自动锁定为Ⅰ→F、Ⅱ→E、Ⅲ→D。
...
- 开局伙伴的人设区在“背景故事”下方提供可选“填写世界书”按钮。开启后可额外填写背景设定；发布时会自动以前面的姓名、种族、身份、层级、性格、喜爱、背景故事作为前序，再把作者填写内容放入“背景设定”，生成与普通世界书角色相同位置/顺序的角色世界书。未开启，或开启后内容为空时，不生成世界书 artifact。
