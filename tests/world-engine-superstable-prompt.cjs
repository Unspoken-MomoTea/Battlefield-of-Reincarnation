...
assert(prompt.includes("version:21,"),'built-in default prompt should be version 21');
...
const templateMatch=prompt.match(/const DEFAULT_STABILITY_PROMPT_TEMPLATE = `[^`]*`;/);
...
const custom='【自定义自救 · {{阶段}}】稳定={{稳定值}}；规则={{规则}}';
assert.match(render({设置:{世界超稳:false},世界:{稳定:95}},custom),/【自定义自救 · 因果警觉】稳定=95/,'custom stability template must be rendered into the actual stage');
