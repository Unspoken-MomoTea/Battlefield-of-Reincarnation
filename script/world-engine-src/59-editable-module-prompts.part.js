...
传闻只来自“世界侧可传播事实”、已有传播链和既有公开传闻；正文不是直接传播源。私密事实必须先形成目击、公开后果、调查、公告或泄露。公开内容不得超过来源/受众认知，传播按时间与空间扩散。无触发保持原样；空分类、传播复核或新公开事实时按需更新，每个触发每类最多1条。普通行动/战斗本身不触发；传闻失败不重跑整轮。购买、扣款与消费性删除由MVU处理。`})
    ...
    class WorldEditableModulePromptFeature {
        constructor(engine){this.engine=engine;}
        initialize(){
            const engine=this.engine;
            let dirty=false;
            const version=Number(engine.config.worldModulePromptVersion)||0;
            if(version<WORLD_MODULE_PROMPT_VERSION){
                if(engine.config.activePromptDocumentId===BUILTIN_DEFAULT_PROMPT_DOCUMENT.id){
                    engine.config.preset=normalizeEditablePreset(COMPACT_DEFAULT_PRESET);
                    engine.config.corePrompt=COMPACT_CORE_WORLD_RULES;
                    engine.config.macroPrompt=COMPACT_MACRO_PROMPT;
                    engine.config.stabilityPromptTemplate=COMPACT_STABILITY_PROMPT_TEMPLATE;
                    engine.config.modulePrompts=worldModulePromptDefaults();
                }else engine.config.modulePrompts=normalizeWorldModulePrompts(engine.config.modulePrompts);
                engine.config.worldModulePromptVersion=WORLD_MODULE_PROMPT_VERSION;
                dirty=true;
            }else{
                const normalized=normalizeWorldModulePrompts(engine.config.modulePrompts);
                if(!same(normalized,engine.config.modulePrompts)){engine.config.modulePrompts=normalized;dirty=true;}
            }
            if(dirty)engine.saveConfig();
        }
        async modifyRequest(request){
            const rebuilt=appendConfiguredWorldModulePrompts(request.system,this.engine.config.modulePrompts);
    ...
    registerWorldEngineFeature('editable-module-prompts',engine=>new WorldEditableModulePromptFeature(engine));
