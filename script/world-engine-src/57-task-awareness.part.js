...
情报交易由世界引擎生成或刷新；购买、扣款、消费性删除及购买后创建任务由MVU/变量AI处理，世界引擎下一轮只读接续。副本成就、击杀、奖励与惩罚不进入世界推进上下文。`;
    ...
    class WorldTaskAwarenessFeature {
        constructor(engine){this.engine=engine;}
        restoreSelection(catalogue) {
        ...
        async afterCatalogue(result){this.restoreSelection(result);return result;}
        async modifyRequest(request,base) {
            ...
            if(plain(payload.输入语义)){
                try{
                    ...
                    if(plain(semantics))Object.assign(payload.输入语义,semantics);
                }catch(_){}
            }
    ...
    registerWorldEngineFeature('task-awareness',engine=>new WorldTaskAwarenessFeature(engine));
