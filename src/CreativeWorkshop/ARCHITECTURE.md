...
- 自 v1.7.0 起，自更新从“维护页工具”提升为客户端生命周期能力：服务健康检查成功后自动扫描 loader；发现旧 SHA 时弹出更新提示，用户确认后事务式重写 ScriptTree，并自动重新载入 SillyTavern。
- 新客户端在重新载入后从新的固定 SHA 正常启动；旧客户端销毁逻辑仍用于同页重复初始化等兜底场景。
