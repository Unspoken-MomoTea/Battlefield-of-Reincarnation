...
(function () {
    ...
    var BLOOD_FUSION_RULES = {
        same: [ // 同级血统融合
            { name:'完美升阶', weight:20, cls:'r-good', list:['品质+1阶','五维补全至目标品质正常区间','A/B 双优质词条融合','若双方血统均具有形态倾向，可生成1个融合形态'] },
        ...
        diff: [ // 高低级
            { name:'稳定强化', weight:35, cls:'r-good', list:['品质不变','五维增加 B 的 20%','融合 B 一条适配被动','若A/B 血统存在形态，根据融合结果决定继承、改造'] },
    ...
    async function bloodFusionStart(aName, bName) {
        ...
        function bloodFusionBuildPrompt(a, b, mode, result) {
            ...
            return '血统融合渲染引擎。融合结果已由系统按权重 roll 出, 你【不得】自行选择结果或改写概率, 只能按给定结果渲染血统数据。\n'
                ...
                + '    原始属性: {力量: F, 敏捷: F, 体质: F, 精神: F, 魅力: F, ATK: 7, DEF: 5, MATK: 3, MDEF: 3, AP: 1}\n'
                + '    效果: { [词条]: 描述 }\n'
                + '    技能: {\n'
                + '     - 名称: 技能名\n'
                + '       品质: F\n'
    ...
    function renderHeroBar(p, sys, editMode) {
        ...
        var tier = tierRomanOf(p.层级); var tierQ = tierQOfClass(p.层级);
    ...
    function renderInfoTab(sd) {
        ...
        var fa = p.最终属性 || {};
        var lifeTier = p.层级; // 当前生命层级(Ⅰ~Ⅸ), 用于基础属性段位显示
    ...
    function renderNpcList(list, editMode, mode) {
        ...
        list.forEach(function(it) {
            ...
            var tierRoman = tierRomanOf(n.层级); var q = tierQOfClass(n.层级);
    ...
    function shopGetActorLibRaw(rawMarket, actorName) {
        ...
        if (actorName === SHOP_ACTOR_HERO) {
            ...
            if (Array.isArray(rawMarket.血统列表) || Array.isArray(rawMarket.技能列表)
                || Array.isArray(rawMarket.装备列表) || Array.isArray(rawMarket.道具列表) || Array.isArray(rawMarket.升级列表)) {
                return rawMarket;
            }
        }
        return null;
    ...
    function shopRemovePurchasedFromLibrary(statData, cart, actorName) {
        ...
        if (libMap && libMap[actorName]) {
            ...
            if (Array.isArray(statData.商城.装备列表) || Array.isArray(statData.商城.技能列表)
                || Array.isArray(statData.商城.血统列表) || Array.isArray(statData.商城.道具列表) || Array.isArray(statData.商城.升级列表)) {
                lib = statData.商城;
            }
        }
        if (!lib) return;
            ...
            var ok = writeBackMvu(function (statData) {
                ...
                if (refreshActor === SHOP_ACTOR_HERO && !libMap[SHOP_ACTOR_HERO]) {
                    ...
                    if (Array.isArray(market.血统列表) || Array.isArray(market.技能列表)
                        || Array.isArray(market.装备列表) || Array.isArray(market.道具列表) || Array.isArray(market.升级列表)) {
                        oldTop = {
                    ...
                    libMap[SHOP_ACTOR_HERO] = oldTop || { 血统列表:[], 技能列表:[], 装备列表:[], 道具列表:[], 升级列表:[] };
                    // 清除旧顶层冗余字段, 统一迁移到成员商库
                    delete market.血统列表;
                    delete market.技能列表;
                    delete market.装备列表;
