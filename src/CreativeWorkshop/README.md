...
- 世界书角色直接填写人物资料并自动生成世界书条目；开局角色/开局伙伴使用表单填写角色资料，层级仅允许Ⅰ-Ⅲ。血统与技能品质由人物层级自动锁定为Ⅰ→F、Ⅱ→E、Ⅲ→D，不允许作者手动越级。
...
- 安装进开局后仍使用原本的 `type / tags / attrs / effects / desc / consume / cd` 字段，因此会进入正确的装备、道具、技能子分类与最终 MVU 写入流程。
