...
assert.match(zod, /所属对象:[\s\S]{0,260}z\.array\(z\.string\(\)\)/, 'ZOD 必须把所属对象规范为字符串数组');
assert.match(zod, /资产墓碑/, '后台 Schema 必须允许程序保存资产删除墓碑');
assert.match(helper, /所属对象[\s\S]{0,500}(?:includes|indexOf)\(['"]<user>['"]\)/, '自动收菜必须只认所属对象数组里的 <user>');
assert.doesNotMatch(helper, /playerName[\s\S]{0,500}所属对象/, '自动收菜不得再把玩家名当成收菜权限');
...
assert.match(vars, /Array\.isArray\(asset\?\.所属对象\)|所属对象[^\n]*Array/, '正文玩家资产投影必须支持所属对象数组');
