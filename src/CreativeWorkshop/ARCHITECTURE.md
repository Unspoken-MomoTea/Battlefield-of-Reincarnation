...
└─ services/
   ...
   └─ installer/
      ...
      ├─ original-scripts.js   # 原酒馆助手脚本屏蔽/替换记录与安全恢复
      ├─ snapshot.js
      ├─ apply.js
      └─ uninstall.js
             │
...
3. 下载和安装是两个动作。
4. 安装必须可回滚；停用只能清理本项目拥有的资源，并恢复该项目临时屏蔽的原版世界书/脚本。
5. 已公开版本不会因为新草稿或驳回而消失。
6. 作者公开可见性（owner_hidden）与管理员审核状态分离：作者可自助下架/重新上架而不锁死更新；管理员 archived 仍可阻止重新公开。
7. 作者更新以“上一版 bundle + 当前元数据”为基线，不要求重建完整包；新文件按同名 artifact 替换，未改内容自动继承。
8. 发布第二步扫描原版资源时只读取当前真正启用的世界书来源与启用条目，避免把酒馆里未激活的资源塞给作者。
9. production / staging 从资源层彻底隔离。
...
- Repair 只重建当前项目自己拥有的世界书条目、正则、脚本和预设；原版世界书与原脚本冲突都采用“保留玩家编辑、只恢复必要启停状态”的安全策略。
- 多个工坊项目若同时声明同一个原版世界书条目或原脚本，恢复采用 claim 语义：只在最后一个占用项目停用后恢复原内容。
