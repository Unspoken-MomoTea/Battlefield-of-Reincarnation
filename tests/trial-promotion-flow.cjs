...
const fresh=()=>({角色:{层级:'Ⅰ'},系统状态:{是否可试炼:false,试炼已完成:false,是否战斗中:false},设置:{},世界:{名称:'试炼世界'},任务:{列表:{试炼一:{委托方:'晋升试炼',状态:'进行中'}},副本成就:{}}});
...
const failed=finalizeHarness([{key:'试炼一',status:'失败'}]);failed.apply();assert.equal(failed.data.系统状态.试炼已完成,false);
const partial=finalizeHarness([{key:'试炼一',status:'可交付'}]);partial.apply();assert.ok(partial.data.任务.列表.试炼一);assert.equal(partial.data.系统状态.试炼已完成,false);
...
console.log('PASS trial flow: pending/failed/completed settlement, both world modes, low-score promotion button, stale targets, permit ordering, deferred guard and replay protection');
