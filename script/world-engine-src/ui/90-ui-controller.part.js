    ...
    class WorldEngineUIController {
        constructor(engine){this.engine=engine;}
            createPanel() {\n            const engine=this.engine;
                ...
                engine.panel.addEventListener('click',event=>{
                    ...
                    if(a==='close')engine.close();
                        ...
                        engine.panel.querySelectorAll('[data-segment-title],[data-segment],[data-core-prompt],[data-macro-prompt],[data-stability-prompt],[data-structure-prompt],[data-npc-audit-prompt],[data-module-prompt],[data-request-prompt]').forEach(el=>el.readOnly=!engine.promptEditing);
            ...
            render(force) {\n            const engine=this.engine;
                ...
                if(engine.tab==='世界推进'){
                    html+=engine.views.world.render({
                    ...
                    html+=engine.views.people.render({
                    ...
                    html+=engine.views.exploration.render({
                        state,w,events,entries,text,fields,areaSceneBody,exists,details,
                        empty,section,eventCard
                    });
                }else if(engine.tab==='资产'){
                    html+=engine.views.assets.render({
                        s,w,entries,plain,matched,tools,section,text,pill,fields,details,empty,
                        userName,relationNamesByKey,people,nameKey
                    });
                }else if(engine.tab==='世界事件'){
                    html+=engine.views.events.render({events,matched,tools,section,timelineCards,empty});
                }else if(engine.tab==='传闻'){
                    html+=engine.views.rumor.render({s,state,tools,section,entries,matched,text,fields,details,empty,pill});
                }else if(engine.tab==='运行记录'){
                    html+=engine.views.history.render({state,radar,showRadar,exists,section,text,entries,fields,empty,pill});
                }else if(engine.tab==='设置'){
                    html+=engine.views.settings.render({section,text});
                }else if(engine.tab==='提示词预设'){
                    html+=engine.views.prompts.render({text,section,empty});
                }else if(engine.tab==='请求检查'){
                    html+=engine.views.inspector.render({text,section,empty,fields,pill});
                }
            ...
            dispose() {\n            const engine=this.engine;
                ...
                if (engine.panel) engine.panel.remove(); if (engine.style) engine.style.remove();
                if (engine.mount) engine.mount.remove();
            }
    }
