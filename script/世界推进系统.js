...
事件分类只允许“当前事件 / 近期节点 / 宏观节点”。进行中的局部事件应写当前事件；通往下一宏观边界的桥接步骤写近期节点；宏观节点必须改变阶段级世界状态。程序会自动降级明显局部的伪宏观节点，并在宏观数量不足时拒绝本轮进入纠错重试。事件.前因写直接因果依赖；事件.参与者尽量写稳定人物名。程序会把明确出现在事件文本/参与者中的后台人物同步到关联事件，但不要依赖程序猜测隐含关系。
