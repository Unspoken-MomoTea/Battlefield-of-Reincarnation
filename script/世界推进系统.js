    ...
    class SamsaraWorldEngine {
        ...
        render(force) {
            ...
            if(this.tab==='世界推进'){
                ...
                html+=section('资料读取范围','<div class="we-config-row"><label>正文窗口 <input data-floors type="number" min="1" max="100" value="'+(this.config.contextTurns||6)+'"> 层</label><label>读取方式 <select data-activation><option value="respect_activation" '+(this.config.activationMode!=='force_selected'?'selected':'')+'>遵循蓝绿灯</option><option value="force_selected" '+(this.config.activationMode==='force_selected'?'selected':'')+'>强制读取勾选项</option></select></label></div><p class="we-muted">遵循蓝绿灯：蓝灯常驻，绿灯扫描上述正文窗口的关键词；禁用项不读。例外：时间轴未初始化或待发生宏观节点少于3个时，已启用且标题明确属于校历、年表、时间线、大事记、大事件摘要、剧情大纲/章节控制的条目会临时以「宏观时间骨架」读取，补足后恢复正常绿灯。强制模式可纳入普通禁用项，但 [variables]、[mvu_update]、正文额外思考及任务/输出技术条目始终隔离，避免重复协议污染世界推演。</p><div class="we-tools"><button data-action="books">加载 / 刷新目录</button><button data-action="book-all">全选</button><button data-action="book-none">全不选</button></div>'+
