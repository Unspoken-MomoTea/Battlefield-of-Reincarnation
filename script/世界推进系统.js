...
3. 任何字段只要已经精确到某月某日，都必须使用程序可解析的数字月日格式，例如“{yyy}年-{mm}月-{dd}日-{时间段}”。纪年名称可保留，但月份必须写数字；禁止“枯叶之月/寒风之月/第12日”这类人类可读却无法定位数字月份的日期。此规则同时适用于顶层“时间”、事件时间、历史时间、传播时间、地区近期变化等。若只能确定“1024年秋”这类粗粒度时间，则保留粗粒度，不要编造月日。
...
5. 人物/地区等“更新时间”属于派生时间戳。模型负责事实内容，程序会用本轮最终世界时间统一盖章；无需反复抄写世界时间。`;
    ...
    const MACHINE_TIME_DESCRIPTION='精确到月日时必须使用数字月日，例如“{yyy}年-{mm}月-{dd}日-{时间段}”；只能确定季节/阶段时可保留粗粒度。';
    ...
    function assertCalendarCompatibleTimeValue(stat,result,value,label='时间') {
        ...
        throw new Error(label+'格式无法用于日历：'+raw+'。精确到月日时请使用数字月份，例如“{yyy}年-{mm}月-{dd}日-{时间段}”；不要用月份名称替代数字月。');
    }
        ...
        async buildRequest(base) {
            ...
            try{
                ...
                payload.世界时间维护={
                    ...
                    精确日期格式:'顶层时间及所有事件/历史/传播等日期，只要精确到月日就必须数字化，例如“{yyy}年-{mm}月-{dd}日-{时间段}”。禁止用“枯叶之月/寒风之月”等月份名称替代数字月。'
                };
                request.input=JSON.stringify(payload,null,2);
            }catch(_){}
...
世界.时间由世界推进维护：为空时据已确认资料初始化；正文没有实际时间流逝就不改。精确到月日必须用数字月日（如“{yyy}年-{mm}月-{dd}日-{时间段}”）；不确定则保留粗粒度，不编造。不得回退，也不得把未来事件时间当当前时间。人物/地区更新时间由程序统一盖章。`}),
