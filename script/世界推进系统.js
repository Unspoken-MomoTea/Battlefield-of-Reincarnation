...
初始化时依据当前设定建立必要的近远期节点；无依据的记录保持空。没有变化就返回仅含摘要的空 WorldResult。世界.因果轨道.当前阶段直接保存一段可读的当前世界局势描述，不再只写“爆发初期/发展期”之类短标签；世界总体局势发生实质变化时，用 WorldResult.因果.当前阶段直接更新这段文字。故事线、下一节点与偏移记录继续作为正文跨章保持方向与因果连续性的只读骨架，因此必须保持简洁、稳定并及时同步重大偏移。正在发生且可能被正文感知的当前事件，必须把公开可见部分写入事件.公开征兆与事件.可见影响；隐藏计划、默认走向、未确认内幕和未来结局仍留在后台其他字段。`;
...
10. 输出分层：模型只提交 WorldResult 业务事实，不生成 JSON Pointer 或 add/replace 路径；程序负责名称归一、增量合并、路径转义、补丁编译、事件分类修复、宏观投影和安全校验。因果.当前阶段是一段直接可读的当前世界局势描述，是“世界动向”的唯一持久化来源；故事线、下一节点、偏移记录会继续作为正文跨章保持方向与因果连续性的只读规划骨架。不要另建公开摘要或正文承接。正文对眼前现实的直接推进信息由程序从进行中的当前事件投影公开征兆与可见影响，因此这些公开字段必须只写已经成为现实、可被合理感知的内容，不得泄露后台秘密或未来节点。`
