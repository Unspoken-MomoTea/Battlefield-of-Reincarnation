...
- 开局角色五维使用与正式开局相同的0-8点→F-SSS映射，总预算8点；开局伙伴总预算16点。两者都只允许1项血统与最多2项技能；伙伴额外可动态添加装备。所有构筑均通过普通表单填写，不暴露 JSON/JS 编辑框。
- 开局商店使用可增删的商品卡片编辑器，每项先选择装备 / 道具 / 技能。品质仅允许 F / E / D，价格限制在 0-1000；装备以表单填写原始属性，道具另外填写数量；每项商品最多2条效果，通过动态“添加效果/删除”按钮维护。
