...
- 世界书角色直接填写人物资料并自动生成世界书条目；开局角色/开局伙伴使用表单填写角色资料，层级仅允许Ⅰ-Ⅲ，原始构筑只允许 1 项血统与最多 2 项技能，不暴露 JSON/JS 编辑框；安装后进入共享 Opening Asset Registry。
- 开局商店使用可增删的商品卡片编辑器，每项先选择装备 / 道具 / 技能。品质仅允许 F / E / D，价格限制在 0-1000；装备以表单填写原始属性，三类商品都以普通字段填写效果与描述，道具另外填写数量。
