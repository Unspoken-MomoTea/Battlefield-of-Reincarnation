    ...
    function worldEventSelect(value,options,field) {
        return '<select data-world-event-field="'+field+'">'+options.map(option=>'<option value="'+fieldEscape(option)+'"'+(String(value)===option?' selected':'')+'>'+fieldEscape(option)+'</option>').join('')+'</select>';
    }
    function fieldEscape(value){return worldEditorEscape(value);}

    class WorldEventEditor {
        ...
        async setRecord(oldName,newName,record) {
            ...
            return this.mutations.persist(stat=>{
                const backend=worldEditorBackend(stat),events=backend.事件||{},current=events[oldName];
        ...
        inlineEditorHtml(name,record) {
            ...
            return '<div class="we-world-editor" data-world-event-edit data-world-event-name="'+fieldEscape(name)+'">'
                +'<div class="we-world-editor-grid">'
                +'<label><span>事件名称</span><input data-world-event-field="name" value="'+fieldEscape(name)+'"></label>'
                ...
                +'<label><span>地点</span><input data-world-event-field="location" value="'+fieldEscape(record?.地点||'')+'"></label>'
                +'<label><span>时间</span><input data-world-event-field="time" value="'+fieldEscape(record?.时间||'')+'"></label>'
                +'<label><span>开始时间</span><input data-world-event-field="start" value="'+fieldEscape(record?.开始时间||'')+'"></label>'
                +'<label><span>预计结束</span><input data-world-event-field="end" value="'+fieldEscape(record?.预计结束||'')+'"></label>'
                +'<label><span>下次检查</span><input data-world-event-field="nextCheck" value="'+fieldEscape(record?.下次检查||'')+'"></label>'
                +'<label><span>更新时间</span><input data-world-event-field="updated" value="'+fieldEscape(record?.更新时间||'')+'"></label>'
                +'<label class="we-world-editor-wide"><span>事件描述</span><textarea data-world-event-field="description">'+fieldEscape(record?.描述||'')+'</textarea></label>'
                +'<label class="we-world-editor-wide"><span>公开征兆</span><textarea data-world-event-field="sign">'+fieldEscape(record?.公开征兆||'')+'</textarea></label>'
                +'<label class="we-world-editor-wide"><span>触发条件</span><textarea data-world-event-field="condition">'+fieldEscape(record?.条件||'')+'</textarea></label>'
                +'<label class="we-world-editor-wide"><span>默认走向</span><textarea data-world-event-field="default">'+fieldEscape(record?.默认走向||'')+'</textarea></label>'
                +'<label class="we-world-editor-wide"><span>已确认结果</span><textarea data-world-event-field="result">'+fieldEscape(record?.结果||'')+'</textarea></label>'
                +'<label><span>前因（每行一个）</span><textarea data-world-event-field="causes">'+fieldEscape(list(record?.前因))+'</textarea></label>'
                +'<label><span>参与者（每行一个）</span><textarea data-world-event-field="participants">'+fieldEscape(list(record?.参与者))+'</textarea></label>'
                +'<label><span>关联任务（每行一个）</span><textarea data-world-event-field="tasks">'+fieldEscape(list(record?.关联任务))+'</textarea></label>'
                +'<label class="we-world-editor-wide"><span>可见影响（JSON 数组）</span><textarea data-world-event-field="impacts">'+fieldEscape(json(record?.可见影响))+'</textarea></label>'
                +'</div><div class="we-world-editor-actions"><button type="button" class="we-world-editor-save" data-action="world-event-save" data-event-name="'+fieldEscape(name)+'">保存修正</button><button type="button" data-action="world-event-cancel">取消</button></div></div>';
        }
        beginEdit(name,card) {
            const record=this.record(name);if(!record||!card)return false;
            card.innerHTML=this.inlineEditorHtml(name,record);card.classList.add('we-world-editing');
        ...
        async saveInline(card,oldName) {
            ...
            const current=this.record(oldName)||{};
            return this.setRecord(oldName,String(value('name')||'').trim(),{
                ...
                前因:worldEditorTextList(value('causes')),参与者:worldEditorTextList(value('participants')),关联任务:worldEditorTextList(value('tasks')),
        ...
        mountModeToggle() {
            ...
            const title=engine.tab==='世界推进'?'事件时间线':'人物名册';
            const section=this.mutations.section(title),head=section?.querySelector('.we-section-head');
            ...
            button.textContent=this.mutations.modeEnabled()?'退出编辑':'编辑模式';
            button.setAttribute('aria-pressed',String(this.mutations.modeEnabled()));
            head.appendChild(button);
        }
        mountControls() {
            const engine=this.engine;
            if(!engine.panel||engine.tab!=='世界推进'||!this.mutations.modeEnabled())return;
            for(const card of engine.panel.querySelectorAll('[data-event-card]')){
                ...
                actions.innerHTML='<button type="button" data-action="world-event-edit" data-event-name="'+fieldEscape(name)+'">编辑</button><button type="button" data-action="world-event-delete" data-event-name="'+fieldEscape(name)+'">删除</button>';
        ...
        ensureStyles() {
            const engine=this.engine;
            if(!engine.style||engine.style.textContent.includes('.we-world-editor-actions{'))return;
        ...
        reportError(error,title='世界事件') {
            const engine=this.engine,message=String(error?.message||error||'世界推进资料编辑失败');
            const toast=engine.host?.toastr||engine.env?.toastr;
        ...
        bindPanel() {
            ...
            panel.addEventListener('click',event=>{
                ...
                if(action==='world-edit-mode'){event.preventDefault();event.stopPropagation();this.mutations.toggleMode();return;}
                ...
                else if(action==='world-event-delete-confirm')task=this.removeRecord(name);
                else if(action==='world-event-delete-cancel')this.cancelDelete(button);
                if(task)Promise.resolve(task).catch(error=>this.reportError(error,'世界事件'));
            });
        }
        afterRender(){this.ensureStyles();this.mountModeToggle();this.mountControls();}
