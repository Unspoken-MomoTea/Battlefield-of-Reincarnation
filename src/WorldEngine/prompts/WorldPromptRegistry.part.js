    ...
    class WorldPromptRegistry {
        constructor(engine){
            ...
            this._definitions=Object.freeze([
                ...
                def({key:'worldTimeInputGuidance',title:'世界时间维护 · 请求内规则',group:'请求内指令',source:'WorldTimeOwnershipFeature / 世界时间维护',scope:'user payload JSON',condition:'每次主世界推进请求',defaultValue:()=>WORLD_PROMPT_WORLD_TIME_INPUT}),
        ...
        rewriteInput(input){
            ...
            if(plain(payload.世界时间维护)){
                let guidance=null;try{guidance=JSON.parse(this.value('worldTimeInputGuidance'));}catch(_){}
                if(plain(guidance)){
                    payload.世界时间维护.所有权=String(guidance.所有权||'');
                    if(plain(payload.世界时间维护.初始化锚定)){
                        payload.世界时间维护.初始化锚定.依据顺序=Array.isArray(guidance.初始化依据顺序)?copy(guidance.初始化依据顺序):[];
                        payload.世界时间维护.初始化锚定.禁止=String(guidance.初始化禁止||'');
                    }
                    payload.世界时间维护.正文时间职责=String(guidance.正文时间职责||'');
                    payload.世界时间维护.精确日期格式=String(guidance.精确日期格式||'');
                    payload.世界时间维护.时间段候选=Array.isArray(guidance.时间段候选)?copy(guidance.时间段候选):[];
                    payload.世界时间维护.推进原则=String(guidance.推进原则||'');
                }
            }
