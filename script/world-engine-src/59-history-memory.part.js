...
只输出 JSON：{"摘要":"..."}`;
        ...
        async requestSummary(world,batch,outputLevel){
            ...
            try{
                const raw=await engine.requestAI(
                    engine.services?.prompts?.historyMemorySystem?.()||HISTORY_MEMORY_SYSTEM,
                    historyMemoryPrompt(world,batch,outputLevel,engine.services?.prompts?.get?.('historyMemoryInputInstruction')||PROMPT_DEFAULT_HISTORY_INPUT),
    ...
    registerWorldEngineFeature('history-memory',engine=>new WorldHistoryMemoryFeature(engine));
